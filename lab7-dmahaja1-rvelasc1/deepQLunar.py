...
class DQNAgent:
    ...
    def train(self, env, episodes, steps, batchSize, batchIterations,\
        ...
        for ep in range(episodes):
            ...
            if ep%50 == 0:
                file_name = ("%s_episode_%d.h5" %(envName,ep))
            ...
            for i in range(steps):
                ...
                if len(self.memory) > batchSize:
                    self.replay(batchSize, i)
                if done:
                    break
            self.history.append(total_reward)
            print(f"This episode {ep} has ended with a reward of {total_reward}")
        ...
        for ep in range(episodes):
            ...
            for i in range(steps):
                ...
                if done:
                    break
            self.history.append(total_reward)
            print("This episode has ended with a reward of",total_reward)
