...
class TSP:
    ...
    def best_neighbor(self, tour):
        ...
        neighbours = self._all_neighbors(tour)
        bestcost = float('inf')
        best_tour = neighbours[0]
        for tour in neighbours:
            tourcost = self.cost(tour)
            if tourcost < bestcost:
                bestcost = tourcost
                best_tour = tour
                # print(bestcost)
        return best_tour, bestcost
    ...
    def random_neighbor(self, tour):
        ...
        tourlen = len(tour)
        citynum = randrange(tourlen)
        movenum = citynum
        while citynum == movenum:
            movenum = randrange(tourlen)
        newtour = tour.move_city(citynum, movenum)
        return newtour, self.cost(newtour) 


if __name__ == '__main__':
    # create a new traveling salesperson problem based on cities in US
    problem = TSP("coordinates/United_States_25.json")
    ...
    print(problem.random_neighbor(tour))
    print(problem.best_neighbor(tour))
