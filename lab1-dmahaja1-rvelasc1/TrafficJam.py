...
def blockingHeuristic(state):
    ...
    cars_blocking = state.findBlocks(pos)
    blocking_heuristic = distance_to_goal + cars_blocking
    return blocking_heuristic
...
def betterHeuristic(state):
    ...
    - incorporates total number of cars on map
    """
    #raise NotImplementedError("TODO")
    pos = state.getPosition()
    distance_to_goal = pos[0]
    number_cars = state.numCars()
    return distance_to_goal + number_cars
...
class TrafficJam:
    ...
    def findBlocks(self, position):
        ...
        for i in range(position[0]):
            unit = self.board[i][position[1]]
            if unit not in listCars:
                listCars.append(unit)
        numBlocks = len(listCars) - 1
        return numBlocks
